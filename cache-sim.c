...
//l1 assoc 
	...
	if (strcmp(l1assoc,"direct")==0)
	{
		...
		l1cache = NewCache(l1size/blocksize);
		l1set = NewSet(1);
...
//l2 assoc 
	...
	if (strcmp(l2assoc,"direct")==0)
	{
		...
		l2cache = NewCache(l2size/blocksize);
		l2set = NewSet(1);
	}
...
//l3 assoc 
	...
	if (strcmp(l3assoc,"direct")==0)
	{
		...
		l3cache = NewCache(l3size/blocksize);
		l3set = NewSet(1);
	}
	...
	while (1)
	{
		...
		if ((l1pass == 0) && (strcmp(l1assoc,"direct")==0))
		{
			...
			l1pass = 0;
		  	...
		  	Set *l1set = NewSet(l1size/blocksize);
		  	l1set = &l1cache->set[set1v];
		...
		if ((l2pass == 0) && (l1pass == 0) && (strcmp(l2assoc,"direct")==0))
		{
			...
			Set *l2set = NewSet(l2size/blocksize);
		  	l2set = &l1cache->set[set2v];
		...
		if ((l3pass == 0) && (l2pass == 0) && (l1pass == 0) && (strcmp(l3assoc,"direct")==0))
		{
			...
			Set *l3set = NewSet(l3size/blocksize);
		  	l3set = &l3cache->set[set3v];
